...
public class AsteroidSuperpowerCollision : MonoBehaviour
{
    ...
    public void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.tag == "Asteroid") {
            switch (GameManager.currentSuperPower) {
                case GameManager.Superpowers.Freeze:
                    AsteroidMovement asteroid = collision.gameObject.GetComponent<AsteroidMovement>();
                    asteroid.FreezeRaySpeedAdjustment();
                    break;
                case GameManager.Superpowers.Fire:                     
